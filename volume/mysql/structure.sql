...
CREATE USER 'rex_prod'@'172.0.0.2' IDENTIFIED BY 'this.is.a.rex.proder';
GRANT SELECT,INSERT,UPDATE ON rex_prod.* TO 'rex_prod'@'172.0.0.2';


#Backup Automation
CREATE USER 'backupclient'@'localhost' IDENTIFIED BY 'this@isbackupPassword';
GRANT LOCK TABLES, SHOW VIEW, RELOAD ON *.* TO 'backupclient'@'localhost'; 
GRANT SELECT ON rex_prod.* TO 'backupclient'@'localhost'; 
...
ALTER TABLE `events` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=0;
ALTER TABLE `sessions` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=0;
ALTER TABLE `sources` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=0;
ALTER TABLE `events` ADD CONSTRAINT `session_link` FOREIGN KEY (`session_id`) REFERENCES `sessions` (`id`), ADD CONSTRAINT `tail_link` FOREIGN KEY (`tail`) REFERENCES `events` (`id`);
ALTER TABLE `sessions` ADD CONSTRAINT `source_c` FOREIGN KEY (`source`) REFERENCES `sources` (`id`);
...
#Structure for Rex-production
#------------------------

CREATE DATABASE IF NOT EXISTS rex_prod;
USE rex_prod;
CREATE TABLE `events` (`id` int(11) NOT NULL,`session_id` int(11) NOT NULL, `datetime` datetime NOT NULL DEFAULT current_timestamp(), `action` varchar(30) NOT NULL,`window` varchar(100) NOT NULL,`tail` int(11) DEFAULT NULL, `extra_data` mediumtext NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CREATE TABLE `passwords` (`name` varchar(30) NOT NULL, `passcode` varchar(100) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
